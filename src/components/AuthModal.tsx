...
export const AuthModal: React.FC<AuthModalProps> = ({ onClose, initialRole = 'cliente' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      if (!phone.trim()) {
        setAuthError('Por favor introduza o seu número de telefone.');
        return;
      }
      if (!password) {
      ...
      if (!forgotIdentifier.trim()) {
        setAuthError('Por favor introduza o seu número de telefone ou e-mail cadastrado.');
        return;
      }
      setIsSubmitting(true);
      setTimeout(() => {
        setIsSubmitting(false);
        setForgotSent(true);
        setSuccessInfo('Instruções de recuperação enviadas. Também pode entrar em contacto com o suporte oficial.');
      }, 800);
    }
  };
      ...
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl border border-slate-200 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          ...
          <div className="flex bg-slate-100 p-1 rounded-2xl my-4">
            ...
            <button
              onClick={() => { setMode('register'); setAuthError(''); setSuccessInfo(''); }}
              className={`flex-1 py-2.5 text-xs font-extrabold rounded-xl transition-all ${
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            <button
              ...
              className={`w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold py-3.5 rounded-xl transition-all shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2 text-xs mt-3 ${
            ...
            {/* Toggle Footer link */}
            <div className="pt-3 text-center border-t border-slate-100 mt-3 text-xs">
              {mode === 'login' ? (
